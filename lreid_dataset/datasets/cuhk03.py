...
class IncrementalSamples4cuhk03(IncrementalPersonReIDSamples):
    ...
    def __init__(self, datasets_root, relabel=True, combineall=False, split_id=0, cuhk03_labeled=True,
        ...
        self.dataset_dir = osp.join(self.root, 'CUHK03_lifelong')
        
        # 检查目录是否存在
        if not osp.exists(self.dataset_dir):
            # 如果不存在，尝试其他可能的路径
            possible_paths = [
                osp.join(self.root, 'cuhk03'),
                osp.join(self.root, 'CUHK03'),
                self.root  # 直接使用根目录
            ]
            for path in possible_paths:
                if osp.exists(path):
                    ...
                    print(f"Using dataset directory: {self.dataset_dir}")
                    break
            else:
                raise FileNotFoundError(f"Cannot find CUHK03 dataset in any expected location. Root: {self.root}")
    ...
    def preprocess_split(self):
        ...
        print(f"Checking preprocess conditions...")
        print(f"imgs_labeled_dir exists: {osp.exists(self.imgs_labeled_dir)}")
        print(f"imgs_detected_dir exists: {osp.exists(self.imgs_detected_dir)}")
