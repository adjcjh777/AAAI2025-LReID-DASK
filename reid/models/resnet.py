...
class Backbone(nn.Module):
    ...
    def forward(self, x, domains=None, training_phase=None, get_all_feat=False,epoch=0, head_only=False):     
        ...
        if get_all_feat is True:
            # We don't usually apply dropout when extracting features for evaluation/KD, 
            # but if it was for training the classifier, we might. 
            # Check usage: get_all_feat=True is used in calculating KL divergence with old model.
            # Typically dropout is part of the model structure before classifier.
            # However, for consistency with training, let's look at `if self.training is False`.
            bn_feat_drop = self.dropout(bn_feat[..., 0, 0])
            cls_outputs = self.classifier(bn_feat_drop)
        ...
        bn_feat = bn_feat[..., 0, 0]
        bn_feat_drop = self.dropout(bn_feat)
        cls_outputs = self.classifier(bn_feat_drop)      
